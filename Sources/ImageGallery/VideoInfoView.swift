...
class VideoInfoView: UIView {
    ...
    private lazy var videoIcon: UIImageView = {
        ...
        videoIcon.frame = CGRect(x: 3, y: 0, width: self.bounds.height, height: self.bounds.height)
        videoIcon.contentMode = .scaleAspectFit
        return videoIcon
    }()
    
    private lazy var videoInfoLabel: UILabel = {
        let frame = CGRect(x: 0, y: 0, width: self.bounds.width - 5, height: self.bounds.height)
        let videoInfoLabel = UILabel(frame: frame)
